...
describe('confirmTestPayment', () => {
  ...
  it('is idempotent: confirming the same intent twice returns the identical receipt', () => {
    const intent = createTestPaymentIntent();
    const first = confirmTestPayment(intent.id);
    const second = confirmTestPayment(intent.id);
    expect(second).toBe(first);
    expect(second.id).toBe(first.id);
    expect(isValidTestReceipt(second)).toBe(true);
  });

  it('does not mint a new receipt when a confirmed intent is re-confirmed with a different card', () => {
    const intent = createTestPaymentIntent();
    const first = confirmTestPayment(intent.id, { last4: '4242' });
    // The charge already succeeded: a later card cannot re-charge it.
    const again = confirmTestPayment(intent.id, { last4: '9999' });
    expect(again).toBe(first);
    expect(again.cardLast4).toBe('4242');
  });

  it('keeps intents independent: two intents still produce two distinct receipts', () => {
    const a = createTestPaymentIntent();
    const b = createTestPaymentIntent();
    const ra = confirmTestPayment(a.id);
    const rb = confirmTestPayment(b.id);
    expect(ra.id).not.toBe(rb.id);
    expect(ra.paymentIntentId).toBe(a.id);
    expect(rb.paymentIntentId).toBe(b.id);
  });

  it('a declined intent stays unconfirmed: the good card can still confirm it once', () => {
    const intent = createTestPaymentIntent();
    expect(() => confirmTestPayment(intent.id, { last4: '0002' })).toThrowError(
      expect.objectContaining({ code: ERROR_CODES.DECLINED })
    );
