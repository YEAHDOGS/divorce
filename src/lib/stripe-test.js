 ...
 *     server-side endpoint reviewed by user, never in this SPA.
...
/**
 * Confirmed intent ids → the single receipt they produced.
 *
 * Real payment systems are idempotent: confirming an already-succeeded
 * intent returns the same result instead of charging twice. This map
 * gives the fixture the same guarantee — a double-clicked Pay button,
 * a retried request, or a modal re-open confirming the same `pi_test_*`
 * id can never mint a second receipt for the same intent.
 */
const confirmedIntents = new Map();
 ...
 *
 * Idempotent: an intent id can only ever produce ONE receipt. Confirming
 * an already-confirmed intent returns the same receipt object it
 * produced the first time (Stripe semantics: re-confirming a succeeded
 * intent does not charge again). A double-clicked Pay button or a
 * retried request therefore cannot mint a second receipt.
 *
...
export function confirmTestPayment(intentId, card = { last4: TEST_CARD.last4 }, options = {}) {
  ...
  // Idempotency: the same receipt, every time, for a confirmed intent.
  const prior = confirmedIntents.get(intentId);
  ...
  receiptCounter += 1;
  // Frozen: the shared reference is handed out on every idempotent
  // re-confirm, so no caller can mutate a stored receipt in place.
  ...
  confirmedIntents.set(intentId, receipt);
  return receipt;
}
